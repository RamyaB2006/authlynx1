...
import BillsRecharge from './BillsRecharge';
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="flex justify-between items-center pb-6 mb-2">
        <div>
          <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">IOB Digital Banking</h1>
      ...
      {/* Always mounted regardless of active tab, so the camera/mic guard
          cycle keeps running uninterrupted while the user browses other
          pages instead of restarting on every tab switch. */}
        ...
        {TABS.map((tab) => (
          <button
            ...
            className={`px-4 py-2.5 rounded-xl text-sm font-bold transition-colors cursor-pointer ${
              activeTab === tab.id ? 'bg-blue-600 text-white shadow-md' : 'text-slate-500 hover:bg-slate-100'
            }`}
          >
            {tab.label}
          </button>
      ...
      {activeTab === 'transactions' && <TransactionHistory userId={currentUserId} accounts={accounts} />}
      {activeTab === 'beneficiaries' && <Beneficiaries userId={currentUserId} />}
      {activeTab === 'bills' && <BillsRecharge userId={currentUserId} accounts={accounts} />}
      {activeTab === 'deposits' && <Deposits userId={currentUserId} accounts={accounts} />}
      {activeTab === 'cards' && <Cards userId={currentUserId} />}
